...
void MedianBackground::UpdateBackground(Mat current_frame) {
    ...
    for (int row = 0; (row < mMedianBackground.rows); row++) {
        for (int col = 0; (col < mMedianBackground.cols); col++) {
            for (int ch = 0; (ch < mMedianBackground.channels()); ch++) {
                int new_value = (mMedianBackground.channels() == 3) ? current_frame.at<Vec3b>(row, col)[ch]
                                                                    : current_frame.at<uchar>(row, col);
                int median = (mMedianBackground.channels() == 3) ? mMedianBackground.at<Vec3b>(row, col)[ch]
                                                                 : mMedianBackground.at<uchar>(row, col);
                ...
                while ((mLessThanMedian[row][col][ch] + mHistogram[row][col][ch][median_bin] < total_divided_by_2) &&
                       (median_bin < 255)) {
                ...
                if (mMedianBackground.channels() == 3)
                    mMedianBackground.at<Vec3b>(row, col)[ch] = (uchar) (median_bin * mValuesPerBin);
                else mMedianBackground.at<uchar>(row, col) = (uchar) (median_bin * mValuesPerBin);
            }
        }
    }
